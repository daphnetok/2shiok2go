...
export default {
  name: 'ListingGrid',
  components: { ListingCard, LoadingSpinner, ToastNotification },
  props: {
    priceOrder: {
      type: String,
  ...
  setup(props) {
    ...
    const hasShownToast = ref(false); // Track if we've already shown the toast
    ...
    // Compute hawkers with active items
    const hawkersWithActiveItems = computed(() => {
      return allHawkers.value.filter(h => hasActiveItems(h));
    });

    // Compute hawkers within 5km
    const hawkersWithin5km = computed(() => {
      return hawkersWithActiveItems.value.filter(hawker => {
        const distance = hawker.distance;
        if (distance === 'N/A') return false;
        return distance <= 5; // 5km or less
      });
    });

    // Watch for hawkers within 5km and show toast
    watch(
      hawkersWithin5km,
      (newHawkers) => {
        // Only show toast once when data first loads and we have hawkers
        if (!hasShownToast.value && !loading.value && newHawkers.length > 0) {
          const count = newHawkers.length;
          toastMessage.value = count === 1 
            ? `Great news! There's ${count} new hawker near you!`
            : `Great news! There are ${count} new hawkers near you!`;
          showToast.value = true;
          hasShownToast.value = true; // Prevent showing again
        }
      },
      { immediate: true }
    );
